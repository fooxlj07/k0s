...
func (s *DualstackSuite) SetupSuite() {
	...
	createdTargetPod, err := kc.CoreV1().Pods(metav1.NamespaceDefault).Create(s.Context(), &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{{Name: "nginx-worker0", Image: "docker.io/library/nginx:1.31.0-alpine"}},
	...
	sourcePod, err := kc.CoreV1().Pods(metav1.NamespaceDefault).Create(s.Context(), &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			Containers: []corev1.Container{{Name: "alpine", Image: "docker.io/library/nginx:1.31.0-alpine"}},
			NodeSelector: map[string]string{
